    ...
    <div class="c_member">
        ...
        <div class="c_inner">
            ...
            <div class="c_title">
                会 员 管 理 
                <!-- 搜索 -->
                <input class="c_sousuo" type="text" placeholder="搜索会员" @change="inpChange">
            ...
            <div class="c_infoitem" ref="c_infoitem" v-for="(item,index) in vipinfoArr" :key="index">
                ...
                <div class="c_infoname">
                    ...
                    <span class="c_change" ref="c_change_name">
                        <!-- 修改信息事件 -->
                        <input ref="name_inp" type="text" @blur="getNewName(index,$event)" />
                    </span>
                </div>
                <!-- 会员手机号 -->
                <div class="c_infonumber">
                    ...
                    <span class="c_change" ref="c_change_num">
                        <input ref="num_inp" type="text" @blur="getNewNum(index,$event)" />
                    </span>
                </div>
                <!-- 会员积分 -->
                <div class="c_infoscore">
                    ...
                    <span class="c_change" ref="c_change_score">
                        <input ref="score_inp" type="text" @blur="getNewScore(index,$event)" />
                    </span>
                </div>
                <!-- 管理员设置 -->
                <div @dblclick="setVipUser(index)" ref="setVip" style="cursor: pointer;">{{item.isVip==1?'取消管理员':'设为管理员'}}</div>
                <!-- 删除会员 -->
                <div class="c_infomove" @click="moveVip(index)">删除</div>
            </div>
        </div>
    </div>
</template>
...
export default {
  ...
  methods: {
    ...
    // 获取增加的 number
    getNum(e) {
      var ze = /^1[34578]\d{9}$/.test(e.target.value);
      if (ze || e.target.value == "") {
    ...
    sureAdd() {
      ...
      if (!(this.name == "" || this.number == "" || this.score == "")) {
        ...
        addMemberInfo(this.name, this.number, score);
        location.reload();
      }
      this.$refs.c_add.style.display = "none";
    },
